...
for i in range(len(Train)):
    ...
    for j in range(len(patches)):
        fd, hog_image = hog(patches[j],visualize=True)
        x=hog_image.ravel()
        Bag.append(x)
        Image.append(x)
...
plt.imshow(a,cmap='gray')
plt.show()
a=np.array(Bag[10]).reshape(16,16)
plt.imshow(a,cmap='gray')
plt.show()
a=np.array(Bag[91]).reshape(16,16)
plt.imshow(a,cmap='gray')
plt.show()
a=np.array(Bag[139]).reshape(16,16)
plt.imshow(a,cmap='gray')
plt.show()
a=np.array(Bag[143]).reshape(16,16)
plt.imshow(a,cmap='gray')
plt.show()
a=np.array(Bag[181]).reshape(16,16)
plt.imshow(a,cmap='gray')
plt.show()
...
Actual_testlabel=np.array(TestLabels)
...
y=list((Actual_testlabel).flatten())
